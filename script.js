...
function search(str) {
	...
	fruits.forEach((fruit) => {
		...
		if (compareFruit.includes(compareInput)){ // if the fruit string contains the search string 
...
function searchHandler(e) {
	if(input.value != '') { //if there is a value input
		let results = search(input.value);
		suggestions.classList.add("has-suggestions");
		showSuggestions(results, input.value);
	} else { // show nothing if blank
...
function showSuggestions(results, inputVal) {
	...
	results.forEach((fruit) => {
		...
		let formattedSuggestion = formatSuggestion(fruit, indexOfSubString, lowerCaseInput.length);
...
function formatSuggestion(fruit, indexOfSub, subStringLength) {
	let suggestion; 
	if(indexOfSub === 0 ) {
		suggestion = "<span class='highlite'>" + fruit.substring(0, subStringLength) + "</span>" + fruit.substring(subStringLength, fruit.length );
	} else {
	}
	suggestion = fruit.substring(0, indexOfSub) + "<span class='highlite'>" + fruit.substring(indexOfSub, indexOfSub+subStringLength) + "</span>" + fruit.substring(indexOfSub+subStringLength, fruit.length );
	return suggestion;
}
